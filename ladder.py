...
N_train_labeled = 100
N_train_unlabeled = 1  # 60000
N_test = 10000
train_l, train_ul, test_set = load_mnist(
        scale=1.0/256.0, shift=0.0, N_train_labeled=N_train_labeled,
...
class VanillaComb(chainer.Chain):
    ...
    def __call__(self, u, z):
        ...
        r += w0u * u
        r += w0zu * z * u
        r += ws * F.sigmoid(b1 + w1z * z + w1u * u + w1zu * z * u)
        return r
...
class Ladder_forward(chainer.Chain):
    ...
    def __call__(self, x, eta, test=False):
        ...
        if self.act is None:
            r = F.broadcast(self.gamma.W, z)[0]
            r *= z + F.broadcast(self.beta.W, z)[0]
            return z, r
        else:
            r = F.broadcast(self.gamma.W, z)[0]
            r *= z + F.broadcast(self.beta.W, z)[0]
...
def loss_unlabeled(x):
    ...
    mus = [enc.bn1.mu, enc.bn2.mu, enc.bn3.mu,
           enc.bn4.mu, enc.bn5.mu, enc.bn6.mu]
    vrs = [enc.bn1.vr, enc.bn2.vr, enc.bn3.vr,
           enc.bn4.vr, enc.bn5.vr, enc.bn6.vr]
    ...
    for i in range(len(zs2)):
        if i == 0:
            ...
            r = (zs2[i] - mus[i-1]) / (vrs[i-1] + 1e-10)
            L += lam[i] * F.mean_squared_error(r, zs3[i])
    return L
...
for epoch in range(len(alpha_plan)):
    ...
    print("classification loss, reconstruction loss = {}, {}".format(
        sum_loss_l/600, sum_loss_ul/600))
    ...
    print("test error, test acc, train error, train acc = {}, {}, {}, {}"
          .format(L.data, acc.data, L_.data, acc_.data))
    sys.stdout.flush()
    # if (epoch+1)%10==0:
